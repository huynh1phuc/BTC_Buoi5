...
public static class Test8
{
    public static bool Run()
    {
        ...
        if (!KiemTra(
            6,
            new double[] { 6.5, 7, 7.9 },
            TaoExpected(0, 3, 0, 0)))
            return false;

        if (!KiemTra(
            7,
            new double[] { 5, 5.5, 6.49 },
            TaoExpected(0, 0, 3, 0)))
            return false;
    ...
    static Dictionary<string, int> TaoExpected(
        int gioi,
        int kha,
        int trungBinh,
        int yeu
    ...
    )
    {
        ...
        Console.WriteLine();
        Console.WriteLine(
            "[FAIL] BÀI 8 - TEST " + soTest
        );

        Console.WriteLine(
            "Xếp loại không đúng Expected."
        );
